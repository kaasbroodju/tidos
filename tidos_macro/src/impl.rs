...
impl ToTokens for ControlTag {
	fn to_tokens(&self, tokens: &mut TokenStream) {
		match self {
			ControlTag::IfChain { if_statement, if_content, if_else_chain, else_content } => {
				ControlTag::to_tokens_if_chain(if_statement, if_content, if_else_chain, else_content, tokens);
			}
			ControlTag::For { left_side, right_side, contents} => {
				ControlTag::to_tokens_for_loop(left_side, right_side, contents, tokens);
			}
			ControlTag::Match { match_statement, cases } => {
...
impl ControlTag {
	fn to_tokens_for_loop(left_side: &Vec<TokenTree>, right_side: &Vec<TokenTree>, contents: &Vec<Content>, tokens: &mut TokenStream) {

		let tokens_children = contents
			.iter()
			.fold(&mut TokenStream::new(), |acc, child| { child.to_tokens(acc); acc })
			.to_owned();
	...
	fn to_tokens_if_chain(if_statement: &Vec<TokenTree>, if_content: &Vec<Content>, if_else_chain: &Vec<(Vec<TokenTree>, Vec<Content>)>, else_content: &Option<Vec<Content>>, tokens: &mut TokenStream) {
	...
	fn to_tokens_match(match_statement: &Vec<TokenTree>, cases: &Vec<(Vec<TokenTree>, Vec<Content>)>, tokens: &mut TokenStream) {


		let cases = cases.iter().map(|(case_statement, case_content)| {
	...
	fn is_static(&self) -> bool {
		match self {
			Content::Tag(element) => {
				...
				let has_only_static_attributes = element.attributes.iter().all(|attribute| attribute.is_static());
				let has_only_static_children = element.children.iter().all(|child| child.is_static());
		...
		match self {
			Content::Tag(html_tag) => {
				...
				if is_component {
					...
					let tag_tokens = quote! { &#component_name { #( #attributes ),* }.to_render(page) };
					...
					if is_self_closing {
						if has_only_static_attributes {
							tokens.append_all(quote! { concat!("<", #tag #(, " ", #static_attributes)* , " />") });
						} else {
							tokens.append_all(quote! { concat!("<", #tag #(, " ", #static_attributes)* ) #(+ " " + #dynamic_attributes)* + " />"});
						}
					} else {
						let mut islands = vec![];
						let mut island = vec![];
						...
						let children = islands
							.iter()
							.map(|(is_static, island)| if *is_static { quote! { concat!( #( #island ),*) } } else { quote! { #( #island )* } })
							.collect::<Vec<_>>();
...
fn custom_element_to_tokens(tag: &str, html_tag: &HTMLTag, tokens: &mut TokenStream) -> TokenStream{
	let attributes = html_tag
		.attributes
		.iter()
		.map(|attribute| {
...
impl Attribute {
	fn is_static(&self) -> bool {
		if self.is_toggle_attribute { return false }
		match &self.value {
			None => { true }
			Some(token) => {
				match token {
					...
					_ => { panic!("Tidos macro error: expected group or ident") }
				}
			}
		}
	}
...
impl ToTokens for Attribute {
	fn to_tokens(&self, tokens: &mut TokenStream) {
		match (&self.value, &self.is_toggle_attribute) {
			...
			(Some(value), false) => {
				let attribute_name = &(&self.name).clone().to_string().trim_start_matches("r#").to_string();
...
pub struct PageWrapper { component: Component }
